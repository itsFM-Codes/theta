param(
    ...
    [string]$FirstAllowDraws = "false",
    [ValidateSet("true", "false")]
    [string]$SecondAllowDraws = "false",
...
if (-not (Test-Path -LiteralPath $theta)) {
    throw "Theta is not built. Run build.cmd release first."
}
if (-not $cutechess) {
...
$pgn = Join-Path $results "theta-selfplay-$stamp.pgn"
$log = Join-Path $results "theta-selfplay-$stamp.log"

$arguments = @(
    "-engine", "name=Theta-A", "cmd=$theta", "dir=$root", "proto=uci", "restart=off",
        "option.Hash=$HashMb", "option.Allow Draws=$FirstAllowDraws",
    "-engine", "name=Theta-B", "cmd=$theta", "dir=$root", "proto=uci", "restart=off",
        "option.Hash=$HashMb", "option.Allow Draws=$SecondAllowDraws",
    ...
    "-maxmoves", "160", "-recover", "-ratinginterval", "10",
    "-pgnout", $pgn, "fi"
)

"Cute Chess: $cutechess" | Tee-Object -FilePath $log
"Settings: games=$Games tc=$TimeControl concurrency=$Concurrency hash=${HashMb}MB firstAllowDraws=$FirstAllowDraws secondAllowDraws=$SecondAllowDraws openings=$OpeningOrder" |
